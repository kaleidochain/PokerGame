"use strict";
cc._RF.push(module, 'a488egcdINIu4umqL41RFNS', 'RechargePopup');
// scripts/components/RechargePopup.js
...
cc.Class({
  ...
  onLoad: function onLoad() {
    ...
    cc.find("bg_recharge/RechargeContentLayer/ExchangeChips/layout/exchangeUSDTNum", this._rechargePopup).getComponent(cc.Label).string = cc.dgame.settings.exchangeRate / 10; //this._exchangeRate = 1.5;   //chips/KAL，15chips/10KAL
    //DepositKAL
    ...
    cc.dgame.utils.addTextChangedEvent(this._editboxChipsNum.node, this.node, "RechargePopup", "onChipsNumTextChanged"); //WithdrawKAL
  ...
  _updateData: function _updateData() {
    ...
    cc.find("bg_recharge/RechargeContentLayer/ExchangeChips/layout/exchangeChipsNum", this._rechargePopup).getComponent(cc.Label).string = Math.floor(10 * cc.dgame.settings.exchangeRate);
    cc.find("bg_recharge/RechargeContentLayer/ExchangeChips/layout/exchangeUSDTNum", this._rechargePopup).getComponent(cc.Label).string = cc.dgame.settings.exchangeRate / 10; //DepositKAL
    ...
    cc.find("bg_recharge/RechargeContentLayer/WithdrawChips/layout/exchangeChipsNum", this._rechargePopup).getComponent(cc.Label).string = Math.floor(10 * cc.dgame.settings.exchangeRate);
    cc.find("bg_recharge/RechargeContentLayer/WithdrawChips/layout/exchangeUSDTNum", this._rechargePopup).getComponent(cc.Label).string = cc.dgame.settings.exchangeRate / 10; //WithdrawKAL

    cc.find("bg_recharge/RechargeContentLayer/WithdrawKAL/layout/KALBalanceNum", this._rechargePopup).getComponent(cc.Label).string = cc.dgame.settings.account.KAL; //KAL余额

    Log.Trace("_updateData cc.dgame.settings.account.KAL: " + cc.dgame.settings.account.KAL); //HandselChips
  ...
  _showRechargePopup: function _showRechargePopup(rechargeType) {
  ...
  _onSelectRecharge: function _onSelectRecharge(index) {
    this._updateData();

    for (var i = 0; i < this._rechargeBackgrounds.length; i++) {
      if (index == i) {
  ...
  onRechargeTypeClicked: function onRechargeTypeClicked(toggle) {
  ...
  onBtnShowExchangeChipsClicked: function onBtnShowExchangeChipsClicked() {
    this._showRechargePopup(0);
  },
  onBtnShowDepositKALClicked: function onBtnShowDepositKALClicked() {
    this._showRechargePopup(1);
  },
  onBtnShowHandselClicked: function onBtnShowHandselClicked() {
    this._showRechargePopup(4);
  },
  _onExchangeGoldcoin: function _onExchangeGoldcoin(data) {
    Log.Trace('[_onExchangeGoldcoin] ' + data);

    this._scheduleRefreshBalance();
  },
  onBtnExchangeChipsClicked: function onBtnExchangeChipsClicked() {
  ...
  onBtnCopyAccountAddrClicked: function onBtnCopyAccountAddrClicked() {
    cc.dgame.clipboard.copyToClipboard(cc.dgame.settings.account.Addr);
    cc.dgame.tips.showTips("Copy<br/>Succeed");
  },
  onKALNumTextChanged: function onKALNumTextChanged() {
  ...
  onChipsNumTextChanged: function onChipsNumTextChanged() {
  ...
  onBtnWithdrawChipsClicked: function onBtnWithdrawChipsClicked() {
  ...
  onBtnWithdrawKALClicked: function onBtnWithdrawKALClicked() {
  ...
  onBtnHandselChipsClicked: function onBtnHandselChipsClicked() {
  ...
  onLinkHistoryClicked: function onLinkHistoryClicked() {
    if (cc.sys.isNative) {
      if (cc.sys.isMobile) {
  ...
  onLinkDigifinexClicked: function onLinkDigifinexClicked() {
    if (cc.sys.isNative) {
      if (cc.sys.isMobile) {
        if (cc.sys.os === cc.sys.OS_IOS) {
  ...
  onBtnConfirm: function onBtnConfirm() {
  ...
  _onExchangeKal: function _onExchangeKal(data) {
    Log.Trace('[_onExchangeKal] ' + data);

    this._scheduleRefreshBalance();
  },
  _doWithdrawChips: function _doWithdrawChips() {
    var exchangeKAL_cmd = {
  ...
  _onKalTransaction: function _onKalTransaction(data) {
    Log.Trace('[_onKalTransaction] ' + data);

    this._scheduleRefreshBalance();
  },
  _doWithdrawKAL: function _doWithdrawKAL() {
    var transfer_cmd = {
  ...
  _onGoldTransaction: function _onGoldTransaction(data) {
    Log.Trace('[_onGoldTransaction] ' + data);

    this._scheduleRefreshBalance();
  },
  _doHandselChips: function _doHandselChips() {
    var transfer_cmd = {
  ...
  _scheduleRefreshBalance: function _scheduleRefreshBalance() {
