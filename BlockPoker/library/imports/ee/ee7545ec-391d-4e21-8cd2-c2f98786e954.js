"use strict";
cc._RF.push(module, 'ee754XsOR1OIYzSwvmHhulU', 'HTTP');
// scripts/Net/HTTP.js
...
var HTTP = cc.Class({
  ...
  statics: {
    ...
    sendRequest: function sendRequest(path, data, handler, extraUrl) {
      ...
      if (cc.sys.isNative) {
        xhr.setRequestHeader("Accept-Encoding", "gzip,deflate", "text/html;charset=UTF-8"); //xhr.setRequestHeader("Host","enode.kalscan.io");
      }
      ...
      xhr.onreadystatechange = function () {
        ...
        if (xhr.readyState === 4 && xhr.status >= 200 && xhr.status < 300) {
          ...
          try {
            ...
            Log.Trace("err:" + e); //handler(null);
          } finally {
            if (cc.vv && cc.vv.wc) {//       cc.vv.wc.hide();    
            }
          }
        }
      };

      if (cc.vv && cc.vv.wc) {//cc.vv.wc.show();
      }
