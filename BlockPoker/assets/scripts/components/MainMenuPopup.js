...
cc.Class({
    ...
    properties: {
        ...
        _buyinChipsDialog: cc.Node,       //带入金币对话框
        _insurancePopup: cc.Node,         //保险说明对话框
        _checkoutExitToast: cc.Node,      //结算离桌提示框
    ...
    onLoad () {
        ...
        this._buyinChipsSlider.node.on('slide', function(event) {
            let buyinChipsMinVal = cc.dgame.utils.getOriginValue(this._buyinChipsMin);
            let buyinChipsMaxVal = cc.dgame.utils.getOriginValue(this._buyinChipsMax);
            ...
            let buyNum = buyinChipsMinVal + Math.round(this._buyinChipsSlider.progress * (buyinChipsMaxVal - buyinChipsMinVal));
            //系统创建token桌或俱乐部token桌
            ...
            let buyinChipsNumVal = cc.dgame.utils.getOriginValue(this._buyinChipsNum);
    ...
    onBtnCloseMainMenuClicked () {
        this._mainMenuPopup.active = false;
    },

    onBtnBuyinChipsClicked () {
        ...
        if (cc.dgame.tableInfo.TableId < 0xE000000000000 && (cc.dgame.tableInfo.TableProps &0x1) == 0) {    //俱乐部积分桌不显示
            cc.find("main_shaixuanbg/layout", this._buyinChipsDialog).active = false;
        }
        let buyinChipsMinVal = cc.dgame.utils.getOriginValue(this._buyinChipsMin);
        let buyinChipsMaxVal = cc.dgame.utils.getOriginValue(this._buyinChipsMax);
        let buyNum = buyinChipsMaxVal / 2;
    ...
    onBtnCloseBuyinChipsPopupClicked () {
    ...
    onBtnCloseInsurancePopupClicked () {
        this._insurancePopup.active = false;
    },

    onBtnInsuranceClicked () {
        for (let i = 0; i < cc.dgame.tableInfo.InsuranceOdds.length && i < this._insuranceOdds.length; i++) {
            this._insuranceOdds[i].string = cc.dgame.tableInfo.InsuranceOdds[i];
        }
        this._insurancePopup.active = true;
    },

    onBtnCheckoutExitClicked () {
        this._mainMenuPopup.active = false;
    ...
    disableCheckoutExit () {
        let checkoutExitTable = cc.find("bg_recharge/CheckoutExit", this._mainMenuPopup).getComponent(cc.Button);
    ...
    // update (dt) {},
    updateStatus() {
        let buyinGold = cc.find("bg_recharge/BuyinGold", this._mainMenuPopup).getComponent(cc.Button);
        let checkoutExitTable = cc.find("bg_recharge/CheckoutExit", this._mainMenuPopup).getComponent(cc.Button);
        let exitTable = cc.find("bg_recharge/Exit", this._mainMenuPopup).getComponent(cc.Button);
        ...
        if (cc.dgame.tableInfo.ContractStatus >= Types.ContractStatus.PLAYING) {
            ...
            if (!!this.playerMyself) {
                ...
                if (player.getStack() < cc.dgame.tableInfo.buyinGoldMin) {
                    //withDrawGold.interactable = false;
                }
            }
        } else {
            ...
            if (!!this.playerMyself) {
                let player = this.playerMyself.getComponent('Player');
                if (player.getStack() < 5 * cc.dgame.tableInfo.BlindInfo.SmallBlindBet + cc.dgame.tableInfo.BlindInfo.TotalAnteBet) {
                    buyinGold.interactable = true;
    ...
    enable () {
        let btnMainMenu = cc.find("Canvas/MenuLayer/LeftTopMemuLayer/BtnMainMenu");
        let btnGameReview = cc.find("Canvas/MenuLayer/LeftTopMemuLayer/BtnGameReview");
        let btnClubResults = cc.find("Canvas/MenuLayer/LeftTopMemuLayer/BtnClubResults");
        if (!!btnMainMenu) {
