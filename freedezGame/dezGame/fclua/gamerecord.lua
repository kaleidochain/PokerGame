...
function gamerecord:addTurnInfo(tinfo)
    ...
    if json.encode(tinfo) == nil then
        log.Warn("json.encode(tinfo)", "=nil")
        return
    end
    ...
    gameReplayPlayerAction["Tick"] = os.time()
...
function gamerecord:addGameOver(settleInfo)
    ...
    local gameReplayPlayerAction = json.decode(json.encode(self.actions[#self.actions]))
    ...
    if holeCardsInfo ~= nil then
        for i, v in ipairs(self.actions) do
            if v.TableId == holeCardsInfo.TableId and v.Hand == holeCardsInfo.Hand then
                ...
                if v.PlayerInfo[tostring(myseat)].HandCard ~= nil then
