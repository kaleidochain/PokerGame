"use strict";
cc._RF.push(module, '3d799ATCqJDK5jMY3p3DRNq', 'CreateTable');
// scripts/Game/CreateTable.js
...
cc.Class({
  ...
  properties: {
    ...
    //盲注设置
    blindInfo: cc.Label,
    //大小盲，共11档，1、2、5、10、20、25、50、100、200、300、500、1k
    buyinGoldMin: cc.Label,
    //最小带入金币数，100BB*最小倍数
    smallBlindSlider: cc.Slider,
    //小盲注滑动条
    smallBlindProgress: cc.ProgressBar,
    //小盲注滑块进度条
    smallBlindSliderBtn: cc.Node,
    //小盲注滑动条滑块
    //人数设置
    maxPlayersSlider: cc.Slider,
    //人数设置滑动条
    maxPlayersSliderBtn: cc.Node,
    //人数设置滑动条滑块
    maxPlayersNodes: [cc.Label],
    //人数设置各档节点
    //牌局时长设置
    gameLengthSlider: cc.Slider,
    //牌局时长设置滑动条
    gameLengthSliderBtn: cc.Node,
    //牌局时长设置滑动条滑块
    gameLengthNodes: [cc.Label],
    //牌局时长设置各档节点
    ...
    slots8AnteSlider: cc.Slider,
    //8档前注设置滑动条
    slots9AnteSlider: cc.Slider,
    //9档前注设置滑动条
    slots10AnteSlider: cc.Slider,
    //10档前注设置滑动条
    slots8AnteSliderBtn: cc.Node,
    //8档前注设置滑动条滑块
    slots9AnteSliderBtn: cc.Node,
    //9档前注设置滑动条滑块
    slots10AnteSliderBtn: cc.Node,
    //10档前注设置滑动条滑块
    slots8AnteNodes: [cc.Label],
    //8档前注
    slots9AnteNodes: [cc.Label],
    //9档前注
    slots10AnteNodes: [cc.Label],
    //10档前注
    //开局人数设置
    slots3AutoStartSlider: cc.Slider,
    //3档开局人数设置滑动条
    slots5AutoStartSlider: cc.Slider,
    //5档开局人数设置滑动条
    slots8AutoStartSlider: cc.Slider,
    //8档开局人数设置滑动条
    slots3AutoStartSliderBtn: cc.Node,
    //3档开局人数设置滑动条滑块
    slots5AutoStartSliderBtn: cc.Node,
    //5档开局人数设置滑动条滑块
    slots8AutoStartSliderBtn: cc.Node,
    //8档开局人数设置滑动条滑块
    slots3AutoStartNodes: [cc.Label],
    //3档开局人数
    slots5AutoStartNodes: [cc.Label],
    //5档开局人数
    slots8AutoStartNodes: [cc.Label],
    //8档开局人数
    //带入金币倍数设置
    buyinGoldMinSlider: cc.Slider,
    //带入金币最小倍数滑动条
    buyinGoldMaxSlider: cc.Slider,
    //带入金币最大倍数滑动条
    buyinGoldProgress: cc.ProgressBar,
    //带入金币倍数进度条
    buyinGoldMinSliderBtn: cc.Node,
    //带入金币最小倍数滑块
    buyinGoldMaxSliderBtn: cc.Node,
    //带入金币最大倍数滑块
    buyinGoldProgressNode: cc.Node,
    //带入金币倍数进度条节点
    buyinGoldTimesNodes: [cc.Label],
    //带入金币倍数各档节点
    //强制盲注设置
    btnStraddleOn: cc.Node,
    //强制盲注开
    btnStraddleOff: cc.Node,
    //强制盲注关
    //多牌设置
    btnRMTOn: cc.Node,
    //多牌开
    ...
    //多牌关
    //保险设置
    insuranceOnLayer: cc.Node,
    //保险开设置图层
    insuranceOffLayer: cc.Node,
    //保险关设置图层
    insuranceTypes: [cc.Toggle],
    //保险类型
    btnCustomOddsSetting: cc.Button,
    //自定义赔率设置按钮
    customOddsSetting: cc.RichText,
    //自定义赔率设置按钮文字
    customOddsSettingLayer: cc.Node,
    //自定义赔率设置图层
    editboxCustomOdds: [cc.EditBox],
    //自定义赔率编辑框
    //等待界面
    normalLoadingLayer: cc.Node
  },
  // LIFE-CYCLE CALLBACKS:
  _formatBlindInfo: function _formatBlindInfo(blindnum) {
    return blindnum >= 1000 ? blindnum / 1000 + 'k' : blindnum;
  },
  _formatGoldNum: function _formatGoldNum(goldnum) {
    goldnum = '' + goldnum;
    return goldnum.length >= 4 ? goldnum.substring(0, goldnum.length - 3) + ',000' : goldnum;
  },
  //根据小盲注滑动条更新盲注信息
  _updateBlindInfo: function _updateBlindInfo() {
  ...
  //根据当前小盲的大小改变前注的档位
  _updateAnteSettings: function _updateAnteSettings() {
    if (this.smallBlind == 1) {
      this.slots8AnteSlider.node.active = true;
      this.slots9AnteSlider.node.active = false;
      this.slots10AnteSlider.node.active = false; //滑块在哪个点上哪个点上的数字颜色高亮

      for (var i = 0; i < this.slots8AnteNodes.length; i++) {
        var anteNode = this.slots8AnteNodes[i];
      ...
      for (var _i = 0; _i < this.slots9AnteNodes.length; _i++) {
        var _anteNode = this.slots9AnteNodes[_i];
        _anteNode.string = this._formatBlindInfo(this.anteArr[this.smallBlind + ''][_i]);

        if (_i == parseInt(this.slots9AnteSlider.progress * (this.slots9AnteNodes.length - 1))) {
          _anteNode.node.color = new cc.color(255, 224, 152, 255);
          this.ante = this.anteArr[this.smallBlind + ''][_i];
        } else {
          _anteNode.node.color = new cc.color(175, 198, 221, 255);
        }
      }
    } else {
      this.slots8AnteSlider.node.active = false;
      this.slots9AnteSlider.node.active = false;
      this.slots10AnteSlider.node.active = true; //滑块在哪个点上哪个点上的数字颜色高亮

      for (var _i2 = 0; _i2 < this.slots10AnteNodes.length; _i2++) {
        var _anteNode2 = this.slots10AnteNodes[_i2];
        _anteNode2.string = this._formatBlindInfo(this.anteArr[this.smallBlind + ''][_i2]);

        if (_i2 == parseInt(this.slots10AnteSlider.progress * (this.slots10AnteNodes.length - 1))) {
          _anteNode2.node.color = new cc.color(255, 224, 152, 255);
          this.ante = this.anteArr[this.smallBlind + ''][_i2];
        } else {
          _anteNode2.node.color = new cc.color(175, 198, 221, 255);
        }
      }
    }
  },
  //根据人数设置滑动条更新人数的颜色
  _updateMaxPlayersInfo: function _updateMaxPlayersInfo() {
    //滑块在哪个点上哪个点上的数字颜色高亮
    for (var i = 0; i < this.maxPlayersNodes.length; i++) {
      var playerNode = this.maxPlayersNodes[i];
    ...
    if (this.playerMax == 4) {
      ...
      this.slots8AutoStartSlider.node.active = false; //滑块在哪个点上哪个点上的数字颜色高亮

      for (var _i3 = 0; _i3 < this.slots3AutoStartNodes.length; _i3++) {
        var autoStartNode = this.slots3AutoStartNodes[_i3];

        if (_i3 == parseInt(this.slots3AutoStartSlider.progress * (this.slots3AutoStartNodes.length - 1))) {
          autoStartNode.node.color = new cc.color(255, 224, 152, 255);
          this.playerMin = this.playerMinArr[_i3];
        } else {
      ...
      for (var _i4 = 0; _i4 < this.slots5AutoStartNodes.length; _i4++) {
        var _autoStartNode = this.slots5AutoStartNodes[_i4];

        if (_i4 == parseInt(this.slots5AutoStartSlider.progress * (this.slots5AutoStartNodes.length - 1))) {
          _autoStartNode.node.color = new cc.color(255, 224, 152, 255);
          this.playerMin = this.playerMinArr[_i4];
        } else {
          _autoStartNode.node.color = new cc.color(175, 198, 221, 255);
        }
      }
    } else if (this.playerMax == 9) {
      this.slots3AutoStartSlider.node.active = false;
      this.slots5AutoStartSlider.node.active = false;
      this.slots8AutoStartSlider.node.active = true; //滑块在哪个点上哪个点上的数字颜色高亮

      for (var _i5 = 0; _i5 < this.slots8AutoStartNodes.length; _i5++) {
        var _autoStartNode2 = this.slots8AutoStartNodes[_i5];

        if (_i5 == parseInt(this.slots8AutoStartSlider.progress * (this.slots8AutoStartNodes.length - 1))) {
          _autoStartNode2.node.color = new cc.color(255, 224, 152, 255);
          this.playerMin = this.playerMinArr[_i5];
        } else {
          _autoStartNode2.node.color = new cc.color(175, 198, 221, 255);
        }
      }
    }
  },
  //根据牌局时长设置滑动条更新牌局时长的颜色
  _updateGameLengthInfo: function _updateGameLengthInfo() {
    //滑块在哪个点上哪个点上的数字颜色高亮
    for (var i = 0; i < this.gameLengthNodes.length; i++) {
      var gameLenNode = this.gameLengthNodes[i];
  ...
  _updateBuyinGoldTimesInfo: function _updateBuyinGoldTimesInfo() {
    ...
    this.buyinGoldProgress.progress = (this.buyinGoldMaxSliderBtn.x - this.buyinGoldMinSliderBtn.x) / this.buyinGoldMinSlider.node.width; //滑块在哪个点上哪个点上的数字颜色高亮

    for (var i = 0; i < this.buyinGoldTimesNodes.length; i++) {
      var timesNode = this.buyinGoldTimesNodes[i];
  ...
  onLoad: function onLoad() {
    Log.Trace("[CreateTable:onLoad]"); //12档小盲

    this.smallBlindArr = [1, 2, 5, 10, 20, 25, 50, 100, 200, 300, 500, 1000]; //8档带入金币倍数

    this.buyinGoldTimesArr = [0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4]; //3档最大玩家数

    this.playerMaxArr = [4, 6, 9]; //8档牌局时长

    this.gameLengthArr = [3600, 3600 * 1.5, 3600 * 2, 3600 * 2.5, 3600 * 3, 3600 * 4, 3600 * 5, 3600 * 6]; //8档最小玩家数

    this.playerMinArr = [2, 3, 4, 5, 6, 7, 8, 9]; //各盲注对应各档前注
    ...
    this.anteArr['1000'] = [0, 250, 500, 1000, 2000, 4000, 8000, 16000, 20000, 30000]; //牌桌默认属性

    this.tableProps = 0;
    this.insuranceOdds = [31, 16, 10, 8, 6, 5, 4, 3.5, 3, 2.5, 2.3, 2, 1.8, 1.6, 1.4, 1.3, 1.2, 1.1, 1.0, 0.8];

    for (var i = 0; i < cc.dgame.settings.setting.CustomOdds.length; i++) {
      this.editboxCustomOdds[i].string = cc.dgame.settings.setting.CustomOdds[i];
    } //小盲默认为1


    this.smallBlindSlider.progress = 0;
    this.smallBlind = this.smallBlindArr[parseInt(this.smallBlindSlider.progress * 11)]; //牌桌时长默认2小时
    ...
    this._updateGameLengthInfo(); //带入金币最小倍数默认1倍，最大倍数默认4倍
    ...
    this._updateBuyinGoldTimesInfo(); //强制盲注默认为false


    this.straddle = 0; //盲注进度条跟随盲注滑块
    ...
    var progress = this.smallBlindSlider.progress;
    this.smallBlindSlider.progress = 0;
    ...
    this.smallBlindSliderBtn.on('touchmove', function (event) {
      ...
      if (worldPoint.x >= this.smallBlindSliderBtnEndPos.x) {
        ...
        var _progress = (worldPoint.x - this.smallBlindSliderBtnStartPos.x) / (this.smallBlindSliderBtnEndPos.x - this.smallBlindSliderBtnStartPos.x);

        this.smallBlindSlider.progress = _progress;
      }
    ...
    this.maxPlayersSlider.node.on('slide', function (event) {
      var i = this.gameLengthNodes.length - 1;
    ...
    this.maxPlayersSliderBtn.on('touchmove', function (event) {
      ...
      if (worldPoint.x >= this.maxPlayersSliderBtnEndPos.x) {
        ...
        var _progress2 = (worldPoint.x - this.maxPlayersSliderBtnStartPos.x) / (this.maxPlayersSliderBtnEndPos.x - this.maxPlayersSliderBtnStartPos.x);

        var _i6 = this.maxPlayersNodes.length - 1;

        for (; _i6 > 0; _i6--) {
          if (_progress2 > (_i6 - 0.5) / (this.maxPlayersNodes.length - 1)) {
            this.maxPlayersSlider.progress = _i6 / (this.maxPlayersNodes.length - 1);
            break;
          }
        }
    ...
    this.gameLengthSliderBtn.on('touchmove', function (event) {
      ...
      if (worldPoint.x >= this.gameLengthSliderBtnEndPos.x) {
        ...
        var _progress3 = (worldPoint.x - this.gameLengthSliderBtnStartPos.x) / (this.gameLengthSliderBtnEndPos.x - this.gameLengthSliderBtnStartPos.x);

        var _i7 = this.gameLengthNodes.length - 1;

        for (; _i7 > 0; _i7--) {
          if (_progress3 > (_i7 - 0.5) / (this.gameLengthNodes.length - 1)) {
            this.gameLengthSlider.progress = _i7 / (this.gameLengthNodes.length - 1);
            break;
          }
        }
    ...
    this.slots8AnteSlider.node.on('slide', function (event) {
      var i = this.slots8AnteNodes.length - 1;
    ...
    this.slots8AnteSliderBtn.on('touchmove', function (event) {
      ...
      if (worldPoint.x >= this.slots8AnteSliderBtnEndPos.x) {
        ...
        var _progress4 = (worldPoint.x - this.slots8AnteSliderBtnStartPos.x) / (this.slots8AnteSliderBtnEndPos.x - this.slots8AnteSliderBtnStartPos.x);

        var _i8 = this.slots8AnteNodes.length - 1;

        for (; _i8 > 0; _i8--) {
          if (_progress4 > (_i8 - 0.5) / (this.slots8AnteNodes.length - 1)) {
            this.slots8AnteSlider.progress = _i8 / (this.slots8AnteNodes.length - 1);
            break;
          }
        }
    ...
    this.slots9AnteSlider.node.on('slide', function (event) {
      var i = this.slots9AnteNodes.length - 1;
    ...
    this.slots9AnteSliderBtn.on('touchmove', function (event) {
      ...
      if (worldPoint.x >= this.slots9AnteSliderBtnEndPos.x) {
        ...
        var _progress5 = (worldPoint.x - this.slots9AnteSliderBtnStartPos.x) / (this.slots9AnteSliderBtnEndPos.x - this.slots9AnteSliderBtnStartPos.x);

        var _i9 = this.slots9AnteNodes.length - 1;

        for (; _i9 > 0; _i9--) {
          if (_progress5 > (_i9 - 0.5) / (this.slots9AnteNodes.length - 1)) {
            this.slots9AnteSlider.progress = _i9 / (this.slots9AnteNodes.length - 1);
            break;
          }
        }
    ...
    this.slots10AnteSlider.node.on('slide', function (event) {
      var i = this.slots10AnteNodes.length - 1;
    ...
    this.slots10AnteSliderBtn.on('touchmove', function (event) {
      ...
      if (worldPoint.x >= this.slots10AnteSliderBtnEndPos.x) {
        ...
        var _progress6 = (worldPoint.x - this.slots10AnteSliderBtnStartPos.x) / (this.slots10AnteSliderBtnEndPos.x - this.slots10AnteSliderBtnStartPos.x);

        var _i10 = this.slots10AnteNodes.length - 1;

        for (; _i10 > 0; _i10--) {
          if (_progress6 > (_i10 - 0.5) / (this.slots10AnteNodes.length - 1)) {
            this.slots10AnteSlider.progress = _i10 / (this.slots10AnteNodes.length - 1);
            break;
          }
        }
    ...
    this.slots3AutoStartSlider.node.on('slide', function (event) {
      var i = this.slots3AutoStartNodes.length - 1;
    ...
    this.slots3AutoStartSliderBtn.on('touchmove', function (event) {
      ...
      if (worldPoint.x >= this.slots3AutoStartSliderBtnEndPos.x) {
        ...
        var _progress7 = (worldPoint.x - this.slots3AutoStartSliderBtnStartPos.x) / (this.slots3AutoStartSliderBtnEndPos.x - this.slots3AutoStartSliderBtnStartPos.x);

        var _i11 = this.slots3AutoStartNodes.length - 1;

        for (; _i11 > 0; _i11--) {
          if (_progress7 > (_i11 - 0.5) / (this.slots3AutoStartNodes.length - 1)) {
            this.slots3AutoStartSlider.progress = _i11 / (this.slots3AutoStartNodes.length - 1);
            break;
          }
        }
    ...
    this.slots5AutoStartSlider.node.on('slide', function (event) {
      var i = this.slots5AutoStartNodes.length - 1;
    ...
    this.slots5AutoStartSliderBtn.on('touchmove', function (event) {
      ...
      if (worldPoint.x >= this.slots5AutoStartSliderBtnEndPos.x) {
        ...
        var _progress8 = (worldPoint.x - this.slots5AutoStartSliderBtnStartPos.x) / (this.slots5AutoStartSliderBtnEndPos.x - this.slots5AutoStartSliderBtnStartPos.x);

        var _i12 = this.slots5AutoStartNodes.length - 1;

        for (; _i12 > 0; _i12--) {
          if (_progress8 > (_i12 - 0.5) / (this.slots5AutoStartNodes.length - 1)) {
            this.slots5AutoStartSlider.progress = _i12 / (this.slots5AutoStartNodes.length - 1);
            break;
          }
        }
    ...
    this.slots8AutoStartSlider.node.on('slide', function (event) {
      var i = this.slots8AutoStartNodes.length - 1;
    ...
    this.slots8AutoStartSliderBtn.on('touchmove', function (event) {
      ...
      if (worldPoint.x >= this.slots8AutoStartSliderBtnEndPos.x) {
        ...
        var _progress9 = (worldPoint.x - this.slots8AutoStartSliderBtnStartPos.x) / (this.slots8AutoStartSliderBtnEndPos.x - this.slots8AutoStartSliderBtnStartPos.x);

        var _i13 = this.slots8AutoStartNodes.length - 1;

        for (; _i13 > 0; _i13--) {
          if (_progress9 > (_i13 - 0.5) / (this.slots8AutoStartNodes.length - 1)) {
            this.slots8AutoStartSlider.progress = _i13 / (this.slots8AutoStartNodes.length - 1);
            break;
          }
        }
    ...
    this.buyinGoldMinSliderBtn.on('touchmove', function (event) {
      ...
      if (worldPoint.x >= this.buyinGoldMinSliderBtnEndPos.x) {
        ...
        var _progress10 = (worldPoint.x - this.buyinGoldMinSliderBtnStartPos.x) / (this.buyinGoldMinSliderBtnEndPos.x - this.buyinGoldMinSliderBtnStartPos.x);

        var _i14 = this.buyinGoldTimesNodes.length - 1;

        for (; _i14 > 0; _i14--) {
          if (_progress10 > (_i14 - 0.5) / (this.buyinGoldTimesNodes.length - 1)) {
            this.buyinGoldMinSlider.progress = _i14 / (this.buyinGoldTimesNodes.length - 1);
            break;
          }
        }
    ...
    this.buyinGoldMaxSliderBtn.on('touchmove', function (event) {
      ...
      if (worldPoint.x >= this.buyinGoldMaxSliderBtnEndPos.x) {
        ...
        var _progress11 = (worldPoint.x - this.buyinGoldMaxSliderBtnStartPos.x) / (this.buyinGoldMaxSliderBtnEndPos.x - this.buyinGoldMaxSliderBtnStartPos.x);

        var _i15 = this.buyinGoldTimesNodes.length - 1;

        for (; _i15 > 0; _i15--) {
          if (_progress11 > (_i15 - 0.5) / (this.buyinGoldTimesNodes.length - 1)) {
            this.buyinGoldMaxSlider.progress = _i15 / (this.buyinGoldTimesNodes.length - 1);
            break;
          }
        }
  ...
  onBtnStraddleOnClicked: function onBtnStraddleOnClicked() {
    this.btnStraddleOn.active = false;
    this.btnStraddleOff.active = true;
    this.straddle = 0;
  },
  onBtnStraddleOffClicked: function onBtnStraddleOffClicked() {
    this.btnStraddleOn.active = true;
    this.btnStraddleOff.active = false;
    this.straddle = 1;
  },
  onBtnRMTOnClicked: function onBtnRMTOnClicked() {
    this.btnRMTOn.active = false;
    this.btnRMTOff.active = true;
    this.tableProps &= ~(1 << 1);
  },
  onBtnRMTOffClicked: function onBtnRMTOffClicked() {
  ...
  onBtnInsuranceOnClicked: function onBtnInsuranceOnClicked() {
  ...
  onBtnInsuranceOffClicked: function onBtnInsuranceOffClicked() {
  ...
  _handleCreateTable: function _handleCreateTable(data) {
  ...
  onBtnCreateTableClicked: function onBtnCreateTableClicked() {
    cc.dgame.normalLoading.startInvokeWaiting();
    var subCreateTable_cmd = {
    ...
    if (this.tableProps & 1 << 3) {
      ...
      for (var i = 0; i < this.editboxCustomOdds.length; i++) {
  ...
  onBtnRoomTypeClicked: function onBtnRoomTypeClicked(toggle) {
  ...
  onBtnInsuranceTypeClicked: function onBtnInsuranceTypeClicked(toggle) {
  ...
  onBtnCustomOddsSettingClicked: function onBtnCustomOddsSettingClicked() {
    this.customOddsSettingLayer.active = true;
  },
  onBtnCloseCustomOddsSettingClicked: function onBtnCloseCustomOddsSettingClicked() {
    this.customOddsSettingLayer.active = false;
  },
  onBtnCustomOddsResetClicked: function onBtnCustomOddsResetClicked() {
  ...
  onBtnCustomOddsSaveClicked: function onBtnCustomOddsSaveClicked() {
    for (var i = 0; i < this.editboxCustomOdds.length; i++) {
