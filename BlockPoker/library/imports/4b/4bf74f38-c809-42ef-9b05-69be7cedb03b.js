"use strict";
cc._RF.push(module, '4bf7484yAlC75sFab587bA7', 'TestScene');
// scripts/Game/TestScene.js
...
var GameTable = cc.Class({
  ...
  properties: {
    ...
    actionIdentifter: cc.Node,
    //玩家行动指示标
    replayCurrentRoundPotNum: cc.Label,
    //牌局回放本轮下注筹码与底池的总和，即总底池数
    replaySidePots: [cc.Node],
    //牌局回放边池图层
    replaySidePotNums: [cc.Label],
    //牌局回放边池筹码数
    //4人桌
    ...
    //4人桌各玩家位置
    replayTableFor4Layer: cc.Node,
    //牌局回放4人桌图层
    replayPlayerAnchorsFor4: [cc.Node],
    //牌局回放4人桌各玩家位置
    //6人桌
    ...
    //6人桌各玩家位置
    replayTableFor6Layer: cc.Node,
    //牌局回放8人桌图层
    replayPlayerAnchorsFor6: [cc.Node],
    //牌局回放8人桌各玩家位置
    //8人桌
    ...
    //8人桌各玩家位置
    replayTableFor8Layer: cc.Node,
    //牌局回放8人桌图层
    replayPlayerAnchorsFor8: [cc.Node],
    //牌局回放8人桌各玩家位置
    //9人桌
  ...
  onLoad: function onLoad() {
    ...
    if (cc.winSize.height / cc.winSize.width > 2) {
      ...
      console.log("newNodePT: (" + _newNodePT.x + ", " + _newNodePT.y + ")");
    ...
    for (var i = 0; i < this._maxPlayers; i++) {
      ...
      if (gametable.active) {
        ...
        console.log("player " + i + ", from: (" + PositionData.GameTableLongPlayerAnchorPositions[i].x + ", " + PositionData.GameTableLongPlayerAnchorPositions[i].y + ") to (" + (PositionData.GameTableLongPlayerAnchorPositions[i].x + _newOrg.x) + ", " + (PositionData.GameTableLongPlayerAnchorPositions[i].y + _newOrg.y) + ")");
      }

      var playerNode = cc.instantiate(this.playerPrefab);
      playerAnchor.addChild(playerNode);
      var player = playerNode.getComponent('Player');
      player.initPlayerByTablePos(this._maxPlayers, i); // player.startCountDown(15);

      if (i == 0) {
        var holeCardsLayer = cc.find("Canvas/HoldCardsLayer");
        var selfHeadWorldPT = playerAnchor.convertToWorldSpaceAR(cc.v2(0, 0));
        var newHoleCardsWorldPT = cc.v2(selfHeadWorldPT.x, selfHeadWorldPT.y + 168); //168=牌高度一半+牌和头30+头高度一半

        holeCardsLayer.setPosition(holeCardsLayer.parent.convertToNodeSpaceAR(newHoleCardsWorldPT)); //player.btnBackToPlay.active = true;
  ...
  //自由加注滑动条初始化
  _initFreeRaiseSlider: function _initFreeRaiseSlider() {
  ...
  //重置牌桌信息
  _resetGameTable: function _resetGameTable() {
    ...
    this._dealCardPlayerPosArray = new Array(); //公共牌牌面向下并隐藏

    for (var i = 0; i < this.dealPublicCards.length; i++) {
      var poker = this.dealPublicCards[i].getChildren()[0].getComponent('Poker');
      poker.setFaceUp(false);
      this.dealPublicCards[i].active = false;
    } //边池重置


    for (var _i3 = 0; _i3 < this.sidePots.length; _i3++) {
      this.sidePots[_i3].setPosition(PositionData.SidePotPositions[_i3].x, PositionData.SidePotPositions[_i3].y);

      this.sidePotNums[_i3].string = cc.dgame.utils.formatRichText(0, "#FFBC1D", true, false);
      this.sidePots[_i3].active = false;
    }
  ...
  testCase0: function testCase0() {
    ...
    var _loop = function _loop(i) {
      var playerAnchor = _this.playerAnchors[_this._maxPlayerStr][i];
      ...
      if (i != 0) {
        player.dealCards(0.05 * i, 0.05 * _this._maxPlayers + 0.05 * i, potPos, 0.2);
      } else {
        ...
        player.dealCardsPlayerMyself(0.05 * i, 0.05 * _this._maxPlayers + 0.05 * i, potPos, holeCard1Pos, holeCard2Pos, 0.2, 1);
      ...
      if (i != 0) {
        _this.scheduleOnce(function () {
          var playerARWorldPt = playerAnchor.convertToWorldSpaceAR(cc.v2(0, 0));
          var angle = Math.atan2(sidePotPos.y - playerARWorldPt.y, sidePotPos.x - playerARWorldPt.x) * 180 / Math.PI - 90;
    ...
    for (var i = 0; i < cc.dgame.tableInfo.MaxNum; i++) {
      _loop(i);
    }
  },
  testCase2: function testCase2() {
    var _this2 = this;

    this.securityTips.string = "Hello world"; // 计算宽
    ...
    textWidth = Math.floor(textWidth / 30) * 30; // 计算高
    ...
    this.scheduleOnce(function () {
      ...
      this.currentRoundPotNum.node.opacity = 153; //this.currentPotLayer.active = true;
    }, 4);
    this.scheduleOnce(function () {
      //this.currentPotLayer.active = false;
    ...
    var orgHeight = this.actionIdentifter.height;
    var srcPos = this.actionIdentifter.convertToWorldSpaceAR(cc.v2(0, 0));
    var playerAnchor = this.playerAnchors[this._maxPlayerStr][0];
    var dstPos = playerAnchor.convertToWorldSpaceAR(cc.v2(0, 0));
    ...
    var _loop2 = function _loop2(i) {
      _this2.scheduleOnce(function () {
        ...
        var dstPos = this.playerAnchors[this._maxPlayerStr][(i + 2) % this._maxPlayers].convertToWorldSpaceAR(cc.v2(0, 0));

        var lastPos = this.playerAnchors[this._maxPlayerStr][i].convertToWorldSpaceAR(cc.v2(0, 0));

        var oldVec = lastPos.sub(srcPos);
        var newVec = dstPos.sub(srcPos);
        var newHeight = dstPos.sub(srcPos).mag() + 135 / 2;
        this.actionIdentifter.stopAllActions();
        this.actionIdentifter.height = orgHeight;
        this.actionIdentifter.runAction(cc.spawn(cc.scaleTo(0.2, 1, newHeight / orgHeight), cc.rotateBy(0.2, oldVec.signAngle(newVec) / Math.PI * 180)));
      }, 2 * (i + 1));
    };

    for (var i = 0; i < this._maxPlayers; i += 2) {
      _loop2(i);
    }
  },
  testCase_: function testCase_() {
    cc.dgame.tableInfo = {};
    ...
    this.playerAnchors[this._maxPlayers] = this.playerAnchorsFor8; // this._selfPlayerWin(true);
    // this.scheduleOnce(function () {
    ...
    for (var i = 0; i < 8; i++) {
      var playerAnchor = this.playerAnchors[this._maxPlayers][i];
      var playerNode = cc.instantiate(this.playerPrefab);
      playerAnchor.addChild(playerNode);
      var player = playerNode.getComponent('Player');
      player.initPlayerByTablePos(this._maxPlayers, i);
    ...
    this._dealCardPlayerPosArray.push(0); // this._dealCardPlayerPosArray.push(1);
    ...
    this._recoverDealInfo(JSON.parse(dealInfo)); // this._dealCardPlayerPosArray.push(6);
