"use strict";
cc._RF.push(module, '9c0a7GgSk1HOZD5otDgIh00', 'ClubHall');
// scripts/Game/ClubHall.js
...
cc.Class({
  ...
  properties: {
    ...
    clubLayer: cc.Node,
    //已加入俱乐部后的界面
    joinClubLayer: cc.Node,
    //加入俱乐部界面
    joinClubToastLayer: cc.Node,
    //加入俱乐部确认对话框
    createClubLayer: cc.Node,
    //创建俱乐部界面
    newClubName: cc.EditBox,
    //创建俱乐部输入俱乐部名称
    currentClubID: cc.Label,
    //当前俱乐部ID
    clubNums: [cc.RichText],
    //房间密码
    clubMembersLayer: cc.Node,
    //俱乐部成员界面
    clubMemberItem: cc.Prefab,
    //俱乐部成员头像预制资源
    clubMembersLayout: cc.Layout,
    //俱乐部成员列表
    clubApplicationLayer: cc.Node,
    //俱乐部成员审批界面
    clubApplicationItem: cc.Prefab,
    //俱乐部成员审批预制资源
    clubApplicationLayout: cc.Layout,
    //俱乐部成员审批列表
    btnKickout: cc.Node,
    //俱乐部成员弹窗踢出按钮（群主视角）
    btnDissolve: cc.Node,
    //俱乐部成员弹窗解散按钮（群主视角）
    btnKickoutCancel: cc.Node,
    //俱乐部成员弹窗取消踢出按钮（群主点击踢出按钮后）
    btnKickoutConfirm: cc.Node,
    //俱乐部成员弹窗确认解散按钮（群主点击踢出按钮后）
    btnQuitClub: cc.Node,
    //俱乐部成员弹窗退出按钮（成员视角）
    kickoutMemberToastLayer: cc.Node,
    //俱乐部踢出成员确认对话框
    dissolveClubToastLayer: cc.Node,
    //解散俱乐部确认对话框
    quitClubToastLayer: cc.Node,
    //退出俱乐部确认对话框
    clubMemberNum: cc.Label,
    //俱乐部成员个数
    normalLoadingLayer: cc.Node,
    //等待界面
    operationTipsBackground: cc.Node,
    //操作提示背景
    operationTips: cc.Label,
    //操作提示
  ...
  onLoad: function onLoad() {
    ...
    cc.dgame.roomEventMgr.handleCreateTable = this._handleCreateTable.bind(this); //监听我加入俱乐部
  ...
  _checkMyClubInfo: function _checkMyClubInfo() {
    cc.dgame.net.gameCall(["myClubs", ""], this._onMyClubs.bind(this));
  },
  _onCreateClubThreshold: function _onCreateClubThreshold(data) {
  ...
  onBtnNewGameClicked: function onBtnNewGameClicked() {
    cc.director.loadScene('CreateTable');
  },
  onBtnCreateClubClicked: function onBtnCreateClubClicked() {
    this.createClubLayer.active = true;
  },
  onBtnCloseCreateClubClicked: function onBtnCloseCreateClubClicked() {
    this.createClubLayer.active = false;
  },
  onBtnJoinClubClicked: function onBtnJoinClubClicked() {
    this.joinClubLayer.active = true;
  },
  onBtnCloseJoinClubClicked: function onBtnCloseJoinClubClicked() {
    this.joinClubLayer.active = false;
  },
  onBtnNumpad0Clicked: function onBtnNumpad0Clicked() {
  ...
  onBtnNumpad1Clicked: function onBtnNumpad1Clicked() {
    if (this.roompassword.length < 6) {
  ...
  onBtnNumpad2Clicked: function onBtnNumpad2Clicked() {
    if (this.roompassword.length < 6) {
  ...
  onBtnNumpad3Clicked: function onBtnNumpad3Clicked() {
    if (this.roompassword.length < 6) {
  ...
  onBtnNumpad4Clicked: function onBtnNumpad4Clicked() {
    if (this.roompassword.length < 6) {
  ...
  onBtnNumpad5Clicked: function onBtnNumpad5Clicked() {
    if (this.roompassword.length < 6) {
  ...
  onBtnNumpad6Clicked: function onBtnNumpad6Clicked() {
    if (this.roompassword.length < 6) {
  ...
  onBtnNumpad7Clicked: function onBtnNumpad7Clicked() {
    if (this.roompassword.length < 6) {
  ...
  onBtnNumpad8Clicked: function onBtnNumpad8Clicked() {
    if (this.roompassword.length < 6) {
  ...
  onBtnNumpad9Clicked: function onBtnNumpad9Clicked() {
    if (this.roompassword.length < 6) {
  ...
  onBtnNumpadDeleteClicked: function onBtnNumpadDeleteClicked() {
    this.roompassword.pop();

    this._updateRoomPassword();
  },
  onBtnNumpadResetClicked: function onBtnNumpadResetClicked() {
    this.roompassword = new Array();

    this._updateRoomPassword();
  },
  _foundInClubList: function _foundInClubList(clubID) {
  ...
  _updateRoomPassword: function _updateRoomPassword() {
    this.joinClubID = 0;

    for (var i = 0; i < this.clubNums.length; i++) {
    ...
    if (this.roompassword.length == 6) {
      if (!this._foundInClubList(this.joinClubID)) {
        //不在已有的俱乐部列表中，则申请
        var clubinfo_cmd = {
  ...
  onBtnCreateClubConfirmClicked: function onBtnCreateClubConfirmClicked() {
  ...
  _onCreateClub: function _onCreateClub(data) {
    Log.Trace("[_onCreateClub]"); //this.schedule(this._checkMyClub, 5);
    //this._checkMyClubCount = 0;
  },
  _handleNewClub: function _handleNewClub(clubInfo) {
  ...
  onBtnCreateRoomClicked: function onBtnCreateRoomClicked() {
    cc.director.loadScene("CreateTable");
  },
  onBtnJoinClubApplyClicked: function onBtnJoinClubApplyClicked() {
  ...
  _onJoinClub: function _onJoinClub(data) {
    this._showTips("You have applied to join and wait for the administrator to confirm");
  },
  _handleJoinClubApproved: function _handleJoinClubApproved(clubInfo) {
  ...
  _handleJoinClub: function _handleJoinClub(clubInfo) {
  ...
  onBtnJoinClubCanceClicked: function onBtnJoinClubCanceClicked() {
    this.joinClubToastLayer.active = false;
  },
  onBtnShowClubMembersClicked: function onBtnShowClubMembersClicked() {
  ...
  onBtnCloseClubMembersClicked: function onBtnCloseClubMembersClicked() {
    this.clubMembersLayer.active = false;
  },
  onBtnKickoutClicked: function onBtnKickoutClicked() {
    for (var i = 0; i < this.clubMembersLayout.node.getChildren().length; i++) {
      var clubMember = this.clubMembersLayout.node.getChildren()[i].getComponent("ClubMemberItem");
  ...
  onBtnDissolveClubToastDissolveClicked: function onBtnDissolveClubToastDissolveClicked() {
  ...
  _onCheckCloseClub: function _onCheckCloseClub(data) {
  ...
  onBtnDissolveClubToastCancelClicked: function onBtnDissolveClubToastCancelClicked() {
    this.dissolveClubToastLayer.active = false;
  },
  onBtnKickoutToastConfirmClicked: function onBtnKickoutToastConfirmClicked() {
  ...
  _onKickout: function _onKickout() {
    var clubMembers = new Array();
    ...
    for (var _i = 0; _i < clubMembers.length; _i++) {
      var _clubMember = cc.instantiate(this.clubMemberItem);

      _clubMember.getComponent("ClubMemberItem").init(clubMembers[_i], this.ownerAddr);

      this.clubMembersLayout.node.addChild(_clubMember);

      if (clubMembers[_i] != this.ownerAddr) {
        _clubMember.getComponent("ClubMemberItem").preSelect();
      }
    }
  },
  onBtnKickoutToastCancelClicked: function onBtnKickoutToastCancelClicked() {
    this.kickoutMemberToastLayer.active = false;
  },
  onBtnKickoutCancelClicked: function onBtnKickoutCancelClicked() {
    for (var i = 0; i < this.clubMembersLayout.node.getChildren().length; i++) {
      var clubMember = this.clubMembersLayout.node.getChildren()[i].getComponent("ClubMemberItem");
  ...
  onBtnQuitClubClicked: function onBtnQuitClubClicked() {
    this.quitClubToastLayer.active = true;
  },
  onBtnQuitClubConfirmClicked: function onBtnQuitClubConfirmClicked() {
  ...
  _onQuitClub: function _onQuitClub(data) {
    this.schedule(this._checkQuitClub, 5);
    this._checkQuitClubCount = 0;
  },
  _checkQuitClub: function _checkQuitClub() {
  ...
  _onCheckQuitClub: function _onCheckQuitClub(data) {
  ...
  onBtnQuitClubCancelClicked: function onBtnQuitClubCancelClicked() {
    this.quitClubToastLayer.active = false;
  },
  onBtnDropdownClubListClicked: function onBtnDropdownClubListClicked() {
    var dropdownMenu = cc.find("Canvas/ClubLayer/scrollview");
    var blockLayer = cc.find("Canvas/ClubLayer/sprite_splash");

    if (dropdownMenu.active) {
      dropdownMenu.active = false;
      blockLayer.active = false;
    } else {
      var clubMenu = cc.find("Canvas/ClubLayer/bg_club_menu");
      var menuPos = clubMenu.convertToWorldSpaceAR(cc.v2(0, 0));
      var dropdownPos = dropdownMenu.convertToWorldSpaceAR(cc.v2(0, 0));
  ...
  _onMyClubs: function _onMyClubs(data) {
    ...
    Log.Trace("[ClubHall:_onMyClubs] masterClubID: " + JSON.stringify(this._masterClubIDs)); //监听我是群主的俱乐部ID的加入申请
    ...
    for (var _i2 = 0; _i2 < data.length; _i2++) {
      //自行退出已解散的俱乐部
      if (data[_i2].Dissolved) {
        var quitclub_cmd = {
          ClubID: data[_i2].ClubID
        };
        cc.dgame.net.gameCall(["quitClub", JSON.stringify(quitclub_cmd)], this._onQuitClub.bind(this));
      } else {
        //没解散的俱乐部显示在列表中
        this.clubList.push(data[_i2]);
      }
    }
    ...
    if (!!cc.dgame.settings.account.ClubID && parseInt(cc.dgame.settings.account.ClubID) > 0) {
      ...
      for (var _i3 = 0; _i3 < data.length; _i3++) {
        if (parseInt(cc.dgame.settings.account.ClubID) == parseInt(data[_i3].ClubID)) {
          valid = true;
          break;
        }
      }
    ...
    for (var _i4 = 0; _i4 < this.clubList.length; _i4++) {
      if (cc.dgame.settings.account.Addr == this.clubList[_i4].OwnerAddr) {
        this._isClubMaster = true;

        if (cc.dgame.settings.account.ClubID == this.clubList[_i4].ClubID) {
  ...
  _onNewClub: function _onNewClub(data) {
  ...
  _updateClubMenu: function _updateClubMenu(clubList, isMaster) {
  ...
  updateClubID: function updateClubID(clubID) {
  ...
  _applicationIcon: function _applicationIcon() {
    var notification = cc.find("Canvas/ClubLayer/layout/btn_club_notification"); //本人为选中俱乐部的群主
  ...
  _checkApplications: function _checkApplications() {
  ...
  _onGetJoinClubApplications: function _onGetJoinClubApplications(data) {
    ...
    var newAppliers = cc.find("Canvas/ClubLayer/layout/btn_club_notification/bg_club_nofitication_number");
    var applierNum = cc.find("Canvas/ClubLayer/layout/btn_club_notification/bg_club_nofitication_number/richtext").getComponent(cc.RichText);
  ...
  getMasterByClubID: function getMasterByClubID(clubID) {
    for (var i = 0; i < this.clubList.length; i++) {
  ...
  _onClubMembers: function _onClubMembers(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      var clubMember = cc.instantiate(this.clubMemberItem);
  ...
  showKickoutMemberToast: function showKickoutMemberToast(memberAddr) {
  ...
  _showTips: function _showTips(msg) {
    this.operationTips.string = msg; // 计算宽
    ...
    var textWidth = Math.min(this.operationTips.node.width, 630);
    textWidth = Math.round(textWidth / 42) * 42; // 计算高
    ...
    var textHeight = this.operationTips.node.height;
    ...
    this.operationTipsBackground.parent.runAction(cc.sequence(cc.delayTime(1.5), cc.fadeOut(1.5)));
  },
  onBtnShowClubApplicationClicked: function onBtnShowClubApplicationClicked() {
    this.clubApplicationLayer.active = true;

    this._updateApplicationList(); //for test
  ...
  onBtnCloseClubApplicationClicked: function onBtnCloseClubApplicationClicked() {
  ...
  _onApprove: function _onApprove(data) {
    ...
    for (var i = 0; i < this.clubApplicationLayout.node.getChildren().length; i++) {
      var clubApplication = this.clubApplicationLayout.node.getChildren()[i].getComponent("ClubApplicationItem");
    ...
    for (var _i5 = 0; _i5 < applications.length; _i5++) {
      this.clubApplicationLayout.node.addChild(applications[_i5]);
    }
  ...
  _handleCreateTable: function _handleCreateTable(createTableInfo) {
